...
/**
 * 
 * @On(event = { "Action1" }, entity = "yourName_Number_StudentIDSrv.CustomerMessage")
...
module.exports = async function (request) {
    ...
    try {
        ...
        customerMessage = await SELECT.one.from('YourName_Number_StudentID.CustomerMessage').where({ ID });
        if (!customerMessage) {
    ...
    if (messageCategory === 'Technical') {
        ...
        try {
            ...
            relevantFAQs = await SELECT.from('YourName_Number_StudentID.ProductFAQ')
    ...
    try {
        ...
        await UPDATE('YourName_Number_StudentID.CustomerMessage').set({
